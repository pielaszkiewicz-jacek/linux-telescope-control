    ...
    Eigen::Vector3d &aSpeed)
{
	...
	// Blad pomiaru srednia arytmetyczna
	double sumErr = 0.0;
	for (int i = 0; i < 3; i++) {
...
{
	return {"Attitude Processor - OPTIMAL REQUEST"};
}
    ...
    std::shared_ptr<tlinAttitudeIteration> prevIter, std::shared_ptr<tlinAttitudeIteration>)
{
	...
	if (isFirst) {
		...
		// Zapamietanie stanu
		state.reset(new tlinsAttProcessorTraiadState{D});
		return state;
	}
}
...
{
	return {"Attitude Processor - Q METHOD"};
}
